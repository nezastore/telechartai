...
if __name__ == '__main__':
    # Blok ini mencoba menangani event loop asyncio dengan lebih tangguh.
    try:
        # Coba jalankan fungsi utama asinkron
        asyncio.run(main())
    except RuntimeError as e:
        # Jika RuntimeError terjadi, itu sering berarti event loop sudah berjalan.
        logger.error(f"Terjadi kesalahan runtime: {e}")
        logger.error("Mencoba mendapatkan atau membuat event loop baru...")
        try:
            # Coba dapatkan event loop yang ada
            loop = asyncio.get_event_loop()
            # Jika loop sedang berjalan, coba tutup. Ini mungkin masih gagal jika loop benar-benar macet.
            if loop.is_running():
                logger.warning("Event loop yang ada ditemukan dan sedang berjalan. Mencoba menutupnya.")
                # Menutup loop yang sedang berjalan. Ini adalah titik di mana RuntimeError bisa terjadi lagi
                # jika loop tidak bisa ditutup dengan bersih.
                loop.close()
                logger.warning("Event loop yang ada berhasil ditutup (atau setidaknya dicoba).")
            # Buat event loop baru
            new_loop = asyncio.new_event_loop()
            # Set event loop baru sebagai yang aktif untuk thread saat ini
            asyncio.set_event_loop(new_loop)
            logger.info("Event loop baru dibuat dan diatur. Menjalankan kembali aplikasi.")
            # Jalankan fungsi utama pada event loop yang baru
            new_loop.run_until_complete(main())
        except Exception as inner_e:
            # Menangkap pengecualian jika ada masalah saat mencoba mengelola event loop
            logger.error(f"Gagal mengelola event loop asyncio secara otomatis: {inner_e}")
            logger.error("Solusi terbaik adalah: Pastikan tidak ada proses Python lain yang berjalan dan restart terminal/IDE Anda.")
            logger.error("Jika masalah berlanjut di lingkungan tertentu (misalnya, beberapa IDE), Anda mungkin perlu menginstal 'nest_asyncio' dan menggunakannya: import nest_asyncio; nest_asyncio.apply()")
